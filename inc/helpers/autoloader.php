...
namespace Easy_WP_Modal\Features\Inc\Helpers;
...
function autoloader( $resource = '' ) {
	...
	$resource_path  = false;
	$namespace_root = 'Easy_WP_Modal\Features\\';
	...
	if ( 'inc' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/inc/%s/%s.php', untrailingslashit( EASY_WP_MODAL_FEATURES_PATH ), $directory, $file_name );
	...
	if ( 'tests' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/tests/php/%s.php', untrailingslashit( EASY_WP_MODAL_FEATURES_PATH ), $file_name );
...
spl_autoload_register( '\Easy_WP_Modal\Features\Inc\Helpers\autoloader' );

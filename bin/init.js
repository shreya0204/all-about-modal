...
if (0 === args.length) {
	rl.question("Would you like to setup the plugin? (Y/n) ", (answer) => {
		...
		rl.question(
			"Enter plugin name (shown in WordPress admin)*: ",
			(pluginName) => {
				...
				rl.question("Confirm the Plugin Details (Y/n) ", (confirm) => {
					...
					rl.question(
						"Would you like to run the plugin cleanup? (Y/n) ",
						(cleanup) => {
	...
	(args.includes("--clean") || args.includes("-c")) &&
	1 === args.length
) {
...
const setupPlugin = (pluginName) => {
	...
	const biggestStringLength =
		pluginDetails["Plugin Build Directory URI Constant: "].length +
		"Plugin Build Directory URI Constant: ".length;

	console.log(info.success("\nPlugin Details:"));
	console.log(info.warning("┌" + "─".repeat(biggestStringLength + 2) + "┐"));
		...
		console.log(
			info.warning(
				"│" +
					" " +
					info.success(key) +
					info.message(pluginDetails[key]) +
					" " +
					" ".repeat(
						biggestStringLength - (pluginDetails[key].length + key.length)
					) +
					info.warning("│")
			)
		);
...
const initPlugin = (pluginInfo) => {
	const chunksToReplace = {
		"project name": pluginInfo.pluginNameLowerCase,
		"Project Name": pluginInfo.pluginName,
		"PROJECT NAME": pluginInfo.pluginNameCobolCase,
		"all-about-modal": pluginInfo.kebabCase,
		"Project-Name": pluginInfo.trainCase,
		"PROJECT-NAME": pluginInfo.cobolCase,
		all_about_modal: pluginInfo.snakeCase,
		All_About_Modal: pluginInfo.pascalSnakeCase,
		ALL_ABOUT_MODAL: pluginInfo.macroCase,
		"all-about-modal-": pluginInfo.kebabCaseWithHyphenSuffix,
		"Project-Name-": pluginInfo.trainCaseWithHyphenSuffix,
		"PROJECT-NAME-": pluginInfo.cobolCaseWithHyphenSuffix,
		all_about_modal_: pluginInfo.snakeCaseWithUnderscoreSuffix,
		All_About_Modal_: pluginInfo.pascalSnakeCaseWithUnderscoreSuffix,
		ALL_ABOUT_MODAL_: pluginInfo.macroCaseWithUnderscoreSuffix,
	};
	...
	files.forEach((file) => {
		...
		Object.keys(chunksToReplace).forEach((key) => {
			if (fileName.includes(key)) {
				fileNameToReplace[fileName] = fileName.replace(
					key,
					chunksToReplace[key]
				);
			}
	...
	if (fileContentUpdated || fileNameUpdated) {
		...
		console.log(
			info.success(
				"\nFor more information on how to use this plugin, please visit the following link: " +
					info.warning(
...
const getAllFiles = (dir) => {
	const dirOrFilesIgnore = [".git", ".github", "node_modules", "vendor"];
	...
	files.forEach((file) => {
		...
		try {
			...
			if (content !== fs.readFileSync(filePath, "utf8")) {
				...
				console.log(
					info.success(
						`Updated [${info.message(chunksToReplace)}] ${info.success(
							"to"
						)} [${info.message(newChunk)}] ${info.success(
							"in file"
						)} [${info.message(path.basename(file))}]`
					)
				);
...
const replaceFileName = (files, oldFileName, newFileName) => {
	files.forEach((file) => {
		...
		try {
			...
			console.log(
				info.success(
					`Updated file [${info.message(
						path.basename(filePath)
					)}] ${info.success("to")} [${info.message(
...
const runPluginCleanup = () => {
	const deleteDirs = [".git", "bin/init.js"];
